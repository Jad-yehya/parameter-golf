...
class Hyperparameters:
    ...
    adam_fused = bool(int(os.environ.get("ADAM_FUSED", "1")))
...
def should_use_fused_adam(requested: bool, device: torch.device) -> bool:
    return bool(requested and device.type == "cuda")


def should_use_non_blocking_copy(device: torch.device) -> bool:
    return device.type == "cuda"


def tensor_to_device(tensor: Tensor, device: torch.device, dtype: torch.dtype) -> Tensor:
    return tensor.to(
        device=device,
        dtype=dtype,
        non_blocking=should_use_non_blocking_copy(device),
    )
...
def memory_summary(device: torch.device) -> str:
    ...
    if device.type == "mps":
        ...
        return f"mps memory allocated: {current} MiB driver: {driver} MiB"
    return "memory summary unavailable for cpu"
...
def eval_val(
    ...
    accum_dtype = torch.float64 if device.type == "cuda" else torch.float32
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            local = tensor_to_device(val_tokens[raw_start:raw_end], device, torch.int64)
            x = local[:-1].reshape(-1, args.train_seq_len)
            y = local[1:].reshape(-1, args.train_seq_len)
            with autocast_context(device, next(model.parameters()).dtype):
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,head_mix,attn_gate_proj,attn_gate_w,parallel_resid_lambdas,parallel_post_lambdas,lane_merge",
    ).split(",")
    if pattern
)
...
class DistributedTokenLoader:
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        ...
        return tensor_to_device(x, self.device, torch.int64), tensor_to_device(y, self.device, torch.int64)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if int(attn_out_gate) + int(sparse_attn_gate) > 1:
            raise ValueError("attn_out_gate and sparse_attn_gate are mutually exclusive")
        ...
        if attn_out_gate:
            self.attn_gate_proj = CastedLinear(attn_out_gate_width, num_heads, bias=False)
            self.attn_gate_proj._zero_init = True
        if sparse_attn_gate:
            self.attn_gate_w = nn.Parameter(torch.zeros(num_heads, attn_out_gate_width, dtype=torch.float32))

    @staticmethod
    def _mix_heads(x: Tensor, mix: Tensor) -> Tensor:
        return torch.einsum("ij,bjtd->bitd", mix.to(dtype=x.dtype), x)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.attn_out_gate:
            ...
            gate_in = gate_src[..., : self.attn_out_gate_width].contiguous()
            gate = 2.0 * torch.sigmoid(self.attn_gate_proj(gate_in))
            y = y * gate.transpose(1, 2)[..., None]
        if self.sparse_attn_gate:
            gate_in = x[..., : self.attn_out_gate_width].contiguous()
            gate = torch.sigmoid(
...
class Block(nn.Module):
    ...
    def parallel_forward(self, lane_attn: Tensor, lane_mlp: Tensor, x0: Tensor) -> tuple[Tensor, Tensor]:
        mix = self.resid_mix.to(dtype=lane_attn.dtype)
        attn_read = mix[0][None, None, :] * lane_attn + mix[1][None, None, :] * x0
        attn_out = self.attn_scale.to(dtype=lane_attn.dtype)[None, None, :] * self.attn(
            self.attn_norm(attn_read)
        )
        mlp_out = self.mlp_scale.to(dtype=lane_mlp.dtype)[None, None, :] * self.mlp(
            self.mlp_norm(lane_mlp)
        )
...
class GPT(nn.Module):
    def __init__(
        ...
        if self.parallel_enabled:
            ...
            post = torch.zeros(num_layers, 2, 2, dtype=torch.float32)
            post[:, 0, 0] = 1.0
            post[:, 1, 1] = 1.0
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x0 = x
        skips: list[Tensor] = []
        lane_attn: Tensor | None = None
        lane_mlp: Tensor | None = None
        ...
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                ...
                if lane_attn is None:
                    x = x + skip
                else:
                    lane_attn = lane_attn + skip
            if self.parallel_enabled and block_idx >= self.parallel_start_layer:
                if lane_attn is None:
                    lane_attn = x
                    lane_mlp = x
        ...
        if lane_attn is not None:
            ...
            if self.parallel_final_lane == "attn":
                x = lane_attn
            elif self.parallel_final_lane == "mlp":
                x = lane_mlp
            else:
                merge = torch.clamp(self.lane_merge.to(dtype=x.dtype), 0.0, 1.0)
                x = merge * lane_attn + (1.0 - merge) * lane_mlp
...
def main() -> None:
    ...
    if requested_device == "cuda":
        ...
        if distributed:
            raise RuntimeError("MPS training path is single-process only; run python train_gpt.py")
        if not torch.backends.mps.is_available():
            raise RuntimeError("MPS was requested but is not available")
        device = torch.device("mps")
    elif requested_device == "cpu":
        if distributed:
            raise RuntimeError("CPU training path is single-process only; run python train_gpt.py")
        device = torch.device("cpu")
    else:
        raise ValueError(f"Unsupported DEVICE={args.device!r}; expected cuda, mps, or cpu")
    compute_dtype = resolve_compute_dtype(args.compute_dtype, device)
    compile_enabled = bool(args.torch_compile and device.type == "cuda")
    if compile_enabled:
    ...
    base_model = GPT(
        ...
        iha_lite=args.iha_lite,
        iha_start_layer=args.iha_start_layer,
        iha_mix_v=args.iha_mix_v,
        parallel_start_layer=args.parallel_start_layer,
        parallel_final_lane=args.parallel_final_lane,
        attn_out_gate=args.attn_out_gate,
        attn_out_gate_src=args.attn_out_gate_src,
    ...
    log0(f"device:{device} compute_dtype:{compute_dtype} torch_compile:{compile_enabled} adam_fused:{adam_fused}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(
        "sdp_backends:cudnn=False flash=True mem_efficient=False math=False"
        if device.type == "cuda"
        else f"sdp_backends:{device.type}_default"
    )
    log0(f"attention_mode:gqa num_heads:{args.num_heads} num_kv_heads:{args.num_kv_heads}")
    log0(
        f"iha_lite:{args.iha_lite} iha_start_layer:{args.iha_start_layer if args.iha_start_layer >= 0 else args.num_layers // 2} "
        f"iha_mix_v:{args.iha_mix_v}"
    )
    log0(
        f"parallel_start_layer:{args.parallel_start_layer} parallel_final_lane:{args.parallel_final_lane} "
